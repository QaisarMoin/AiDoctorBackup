...
export default function ClinicLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        login(response.data.token, response.data.user, '/clinic-dashboard');
        toast.success('Login successful! Redirecting to Clinic Admin Dashboard...');
      } else {
  ...
  return (
    <>
      {loading && <Loader message={'Authenticating Admin...'} />}
      ...
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-cyan-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse"></div>

      <div className="relative z-10 w-full max-w-md">
        {/* Header */}
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-3">
            <div className="bg-gradient-to-br from-indigo-600 to-cyan-600 p-3 rounded-2xl shadow-lg">
          ...
          <p className="text-gray-600 text-sm">Secure Portal for Clinic Administrators</p>
        </div>

        {/* Login Card */}
        <div className="bg-white rounded-3xl shadow-2xl p-8 backdrop-blur-xl bg-opacity-95 border border-indigo-50">
          {/* Form */}
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-indigo-600 to-cyan-600 text-white font-semibold py-3 rounded-xl hover:shadow-lg transition-all duration-300 disabled:opacity-70 mt-6"
            >
              {loading ? 'Processing...' : 'Clinic Login'}
            </button>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full border-2 border-gray-200 hover:border-indigo-600 bg-white hover:bg-indigo-50 text-gray-700 font-semibold py-3 rounded-xl transition-all duration-300 flex items-center justify-center gap-3 group disabled:opacity-70"
          >
          ...
          <div className="text-center mt-6 pt-4 border-t border-gray-100">
            <p className="text-sm text-gray-600">
              ...
              <Link to="/clinic-signup" className="text-indigo-600 hover:text-cyan-600 font-semibold transition-colors">
                Register Here
              </Link>
            </p>
          </div>
